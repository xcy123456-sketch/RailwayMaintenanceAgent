...
class KGEdge(BaseModel): # 边类
    ...
    edge_info: Literal["包含", "存在病害","存在阈值"] = Field(description="关系名称")

class State(TypedDict):
    text: str
    node_analysis: str
    node_group: List[KGNode_Defect | KGNode_Infrastructure | KGNode_Threshold]
    edge_analysis: str
    edge_group: List[KGEdge]

class EntityOutput(BaseModel):
    nodes: List[KGNode_Defect | KGNode_Infrastructure | KGNode_Threshold]
...
def generate_node_analysis(state: State):
...
def generate_node_entity(state: State):
...
def generate_edge_analysis(state: State):
    """"Use llm to find potential node items"""
    prompt = ChatPromptTemplate.from_template(
        """你现在要将轨道规范整理成知识图谱，以下是一段规范原文：
        {text}
...
def generate_edge_entity(state: State):
    """Use llm to obtain node entity based on previous analysis."""
    prompt = ChatPromptTemplate.from_template(
        """你现在要将轨道规范整理成知识图谱，以下是一段规范原文：
        {text}
...
workflow.add_edge(START, "generate_node_analysis")
...
workflow.add_edge("generate_edge_entity", END)

chain = workflow.compile()
state = chain.invoke({
    "text": """第5.6.1条 道岔维修作业应符合以下要求：
...
print(state['node_analysis'])
print(state['node_group'])
print(state['edge_analysis'])
print(state['edge_group'])